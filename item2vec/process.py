...
class ItemNameProcessor(object):
    ...
    def __init__(self, data, name_col):
        ...
        self.clean_data = self.map_to_idx()
    ...
    def cut_name(self, name):
        ...
        punc = set(u'，。？！、；：「」『』（）［］〔〕【】—…－-～‧《》〈〉﹏＿')
        exclude = set(string.punctuation) | set(punc)
    ...
    def map_to_idx(self):
        word_to_idx = dict(zip(self.word_list, range(len(self.word_list))))
        self.word_to_idx = word_to_idx
        word_to_idx_fun = lambda x: [word_to_idx[i] for i in x.split('::::') if i in self.word_list]
        return self.data_cut.apply(word_to_idx_fun)
...
# data = pd.read_csv('../datas/item2vec/data.csv')
# processor = ItemNameProcessor(data, name_col='name')
#
# print(processor.get_word_dict())
